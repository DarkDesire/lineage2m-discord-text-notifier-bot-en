...
class Boss(Base):
    ...
    def auto_tagged(self):
        ...
        return(f"{curr_resp_dt.strftime(self.time_pattern)} | **{self.name.upper()}** appeared. Next respawn in {self.respawn_hours}h at {next_dt.strftime(self.time_pattern)}")
    
    def boss_untagged_str(self) -> str:
        return(f"Boss **{self.name.upper()}** deleted from the list!")
               
    def boss_tagged_str(self) -> str:
        (hours, minutes, _) = self.last_time()
        return(f"Boss **{self.name.upper()}** killed!\nI'll remind you of his respawn in {hours}h {minutes}m")

    def boss_status_str(self):
        if self.resp_timestamp is None:
            return(f"**{self.name.upper()}** not logged")
        ...
        if hours == 0 and minutes == 0 or not is_valid:
            return(f"{hours_minutes} | **{self.name.upper()}** in __{self.location_name}__ appeared! | {self.chance}%")
        elif is_valid:
            return(f"{hours_minutes} | **{self.name.upper()}** in __{self.location_name}__ in {hours}h {minutes}m | {self.chance}%")
        else: 
            pass
...
class NoBossFound(Exception):
    def __init__(self, name: str, message: str="There are no bosses with that name"):
        self.name = name
        self.message = message
        super().__init__(self.message)

class ManyBossesReturned(Exception):
    def __init__(self, name: str, bosses: typing.List[str], message: str="Found more than 1 boss"):
        self.name = name
        self.bosses = bosses
        self.message = message
        super().__init__(self.message)